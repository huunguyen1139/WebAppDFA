...
namespace WebApp.production
{
    public partial class MasterPlan : System.Web.UI.Page
    {
        ...
        private void CreateDynamicTableRow()
        {
            ...
            DataTable dt = (Request["showPIpc"] != null && Request["showPIpc"].ToString().Equals("true")) ? SQRLibrary.ReturnDatatablefromSQL("exec [MasterPlan]") : SQRLibrary.ReturnDatatablefromSQL("exec [MasterPlanNoPC]");
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                foreach (DataRow row in dt.Rows)
                {
                    ...
                    for (int i = 0; i < dt.Columns.Count; i++)
                    {
                        ...
                        tbrowcell.Text = dt.Columns[i].ColumnName.Equals("PI") ? "<a href='ProductionStatus?PI=" + row[i].ToString() + "' target='_blank'>" + row[i].ToString() + "</a>" : row[i].ToString();
